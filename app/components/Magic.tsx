...
const MagicCard: React.FC<MagicCardProps> = ({
  ...
  return (
    <div
      ...
      className={cn(
        "relative z-0 size-full rounded bg-border p-4 text-card-foreground shadow-sm",
        "bg-[radial-gradient(var(--mask-size)_circle_at_var(--mouse-x)_var(--mouse-y),#cf3030aa_0,#6d97f8aa_50%,transparent_100%)]",
        className
      )}
      {...props}
    >
      ...
      {/* Background */}
      <div
        className={"absolute inset-px -z-20 rounded bg-[hsl(var(--card)/0.98)]"}
      />
    </div>
  );
};
